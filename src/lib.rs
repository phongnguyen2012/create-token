...
#[near_bindgen]
impl Contract {
    ///khoi tao contract voi tong so token boi ownner_id voi ham metadata
    #[init]
    pub fn new_default_meta(owner_id: AccountId, total_supply: U128) -> Self {
        Self::new(
            ...
            FungibleTokenMetadata {
                ...
                name: "MACHIKO Token".to_string(),
                symbol: "MACHIKO".to_string(),
    ...
    pub fn new(
        ...
        near_contract_standards::fungible_token::events::FtMint{
            owner_id: &&owner_id,
            amount: &&total_supply,
            memo: Some("Tong so token da duoc duc ra"),
        }.emit();
        this
    }
    ...
    fn on_token_burned(&mut self, account_id: AccountId, amount: Balance) {
        log!("Tai khoan @{} burned {}", account_id, amount);
    }
    

}
near_contract_standards::impl_fungible_token_core!(Contract, token, on_token_burned);
...
mod tests {
    ...
    use super::*;
    const TOTAL_SUPPLY: Balance = 1_000_000_000_000_000_000;

    fn get_context(predecessor_account_id: AccountId) -> VMContextBuilder{
        let mut builder = VMContextBuilder::new();
        builder.current_account_id(accounts(0)).signer_account_id(predecessor_account_id.clone())
    ...
    fn test_transfer() {
        ...
        testing_env!(context.storage_usage(env::storage_usage()).attached_deposit(contract.storage_balance_bounds().min.into())
        .predecessor_account_id(accounts(1)).build());

        // dang ky tai khoan
        contract.storage_deposit(None, None);
        testing_env!(context.storage_usage(env::storage_usage()).attached_deposit(1)
        .predecessor_account_id(accounts(2)).build());

        let transfer_amount = TOTAL_SUPPLY / 3;
        contract.ft_transfer(accounts(1), transfer_amount.into(), None);
        testing_env!(context.storage_usage(env::storage_usage()).account_balance(env::account_balance()).is_view(true)
            .attached_deposit(0).build());
